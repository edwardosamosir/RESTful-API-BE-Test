...
describe('adminAuthentication', () => {
    ...
    test('should pass authentication and proceed to next middleware if access token is valid and user is an admin', async () => {
        req.headers.access_token = 'valid-access-token';

        const payload = { id: 1 };
        const userData = { id: 1, role: 'Admin', email: 'admin@example.com' };
        ...
        expect(req.user).toEqual({ id: 1, role: 'Admin', email: 'admin@example.com' });
    ...
    test('should throw Forbidden error if user role is not admin', async () => {
        req.headers.access_token = 'valid-access-token';
